      ...
      try {
        ...
        if (image != null) {
         emit(LoadingprofileState());
          ...
          if (uid != null) {
        final userDoc = await firestore.collection('user details').doc(uid).get();
        if (userDoc.exists) {
          // Update the 'imageUrl' field in the existing user's document
          await firestore.collection('user details').doc(uid).update({'imageUrl': downloadURL});
        } else {
          // Create a new document with the 'imageUrl' field
          await firestore.collection('user details').doc(uid).set({'imageUrl': downloadURL});
        }
      }
